  ...
  var SECTIONS = [
    ...
    },
    {
      ...
      html: [
        ...
        '<dt>Sound</dt><dd>Every sheep can be listened to as well as looked at — see <em>The sound</em>.</dd>',
    ...
    },
    {
      ...
      html: [
        ...
        '<tr><td><kbd>A</kbd></td><td>sound on / off (off by default; also a button in the toolbar, with the volume beside it)</td></tr>',
        ...
        'fit, keep, stream, clear, the quality preset, the details overlay, sound and its volume,',
        'fullscreen and the handbook.</p>',
    ...
    {
      id: 'sound', title: 'The sound', kicker: 'Every sheep is an instrument',
      html: [
        '<p class="lead">Press <kbd>A</kbd>. Every sheep can be listened to as well as looked at, and',
        'nothing is added to a sheep to make that true — the sound is derived from the genome it',
        'already has. A flock saved before any of this existed plays, and two people with the same',
        '<code>.sheep.json</code> hear the same thing.</p>',
        '<p>That one decision is what makes the rest work. The stream already hands out continuous',
        'intermediate genomes, so a morph crossfades with no separate audio interpolation to keep in',
        'step. <code>applyLoop</code> is already periodic in phase, so the sound closes when the',
        'picture closes. And the offline renderer can already say which sheep is on screen at second',
        '<em>t</em>, so the soundtrack of a render is <em>evaluated</em> over the timeline rather than',
        'recorded off a performance.</p>',
        '<h3>A transform is a voice</h3>',
        '<table class="htable">',
        '<thead><tr><th>Genome</th><th>Sound</th></tr></thead><tbody>',
        '<tr><td>weight × opacity</td><td>how loud — how much it draws is how much you hear</td></tr>',
        '<tr><td>affine contraction</td><td>pitch. A map repeating the picture at every scale <em>s</em> repeats the spectrum at the same ratio</td></tr>',
        '<tr><td>the two axis scales</td><td>how far apart they are is spectral tilt and resonance</td></tr>',
        '<tr><td>affine rotation</td><td>where the voice sits in the stereo field</td></tr>',
        '<tr><td>colour</td><td>its hue in the palette, which is its note in the scale</td></tr>',
        '<tr><td>colour speed</td><td>how fast it slides onto that note</td></tr>',
        '<tr><td>variations</td><td>timbre — see below</td></tr>',
        '</tbody></table>',
        '<p>Variations are not looked up as oscillator types. That would be a switch, and a switch',
        'clicks and cannot be interpolated. Each of the 81 is projected onto five continuous axes —',
        'brightness, noise, inharmonicity, resonance and fold — and the variation weights become the',
        'mixing coefficients. Inversive maps like <code>spherical</code> blow up near the origin and',
        'scatter the spectrum the way they scatter the picture; the 19 variations already tagged as',
        'stochastic are the noisy ones; <code>julian</code> and <code>cpow</code> carry a power',
        'parameter, and an <em>n</em>-fold rotational symmetry is an integer frequency ratio. A',
        'negative variation weight draws the shape inside out, so it plays the same note in',
        'anti-phase.</p>',
        '<h3>The sequence is the chaos game</h3>',
        '<p>The picture is drawn by a token hopping between transforms: one chosen with probability',
        'proportional to its weight, with <code>xaos</code> deciding which may follow which. Run the',
        'identical hop at six notes a second instead of a million and every landing is a note — so',
        'the melody and the image are the same process at two rates, not two systems sharing an',
        'app.</p>',
        '<p>It is also what finally makes <code>xaos</code> audible: a sparse matrix is the',
        'difference between a wandering line and a riff. A new random sheep has none, so its sequence',
        'is a plain weighted walk; mutate or breed one and the matrix starts to bite.</p>',
        '<p class="hnote">The walk is seeded from the genome, so the same sheep plays the same riff',
        'on any machine. The step count is a whole number fitted to the loop — the same rule motion',
        'channels follow — so the riff comes round exactly with the picture. Notes are scheduled on',
        'the audio clock, not on frames: the frame loop deliberately loses time on a hitch rather',
        'than jumping, which is right for a picture and would be a dropped note here.</p>',
        '<h3>The palette is the harmony</h3>',
        '<p>Each voice reads its hue at its own colour index, so <b>palette rotate transposes</b> —',
        'and since that is one of the motion channels, a sheep whose palette cycles once per loop',
        'transposes once per loop and lands exactly back where it started. The scale comes from the',
        'palette generator where one was used (mono is a single hue and has nothing to argue about;',
        'triadic is three) and is measured otherwise. <b>Scale</b> on the Sound tab overrides it;',
        '<em>Off</em> leaves the pitches unquantised.</p>',
        '<h3>The image settings are the mixing desk</h3>',
        '<table class="htable">',
        '<thead><tr><th>Image setting</th><th>Sound</th></tr></thead><tbody>',
        '<tr><td>brightness</td><td>output level</td></tr>',
        '<tr><td>gamma, gamma threshold</td><td>compression curve and threshold — both are tone curves over dynamic range</td></tr>',
        '<tr><td>glow</td><td>reverb send: light bleeding out of the bright areas is a tail around loud events</td></tr>',
        '<tr><td>glow radius / threshold</td><td>room size and how dark the tail goes; how much of it there is</td></tr>',
        '<tr><td>vignette</td><td>master low-pass</td></tr>',
        '<tr><td>saturation</td><td>stereo width</td></tr>',
        '<tr><td>symmetry</td><td>detuned copies, fading in with the count</td></tr>',
        '<tr><td>mirror</td><td>one copy phase-inverted — a reflection</td></tr>',
        '<tr><td>grain</td><td>noise floor, the same thing in both media</td></tr>',
        '<tr><td>camera zoom</td><td>filter cutoff: closer is brighter</td></tr>',
        '<tr><td>camera spin</td><td>one pass across the stereo field per revolution, in the direction it turns</td></tr>',
        '</tbody></table>',
        '<h3>Every sheep sets its own pace</h3>',
        '<p>The controls on the Sound tab are a centre, not a value. Each sheep reads three things',
        'about how it is built and moves them: <b>how many transforms</b> it has decides how busy it is',
        'and how spacious the attack; <b>how long its loop is</b> decides that a sheep breathing slowly',
        'does not play fast, and holds a longer tail; <b>how many variations</b> it carries, and whether',
        'it has a final transform, decides how much of it is notes rather than held tone. The palette',
        'hue variance sets the stereo width.</p>',
        '<p class="hnote">Which readings to use was measured rather than guessed. The obvious one — how',
        'evenly the transforms share the draw — sits between 0.96 and 1.00 on almost every random sheep,',
        'a constant with a rounding error.</p>',
        '<p>If a flock still sounds too alike: <b>Kept across new sheep</b> at the top of the Image tab,',
        'on by default, freezes the whole Image tab across everything you generate, and half the mixing',
        'desk with it. Switch it to <em>Reset with each new sheep</em> and every sheep brings its own',
        'glow, symmetry and gamma — its own room and stereo image.</p>',
        '<h3>Tone</h3>',
        '<p>The voices are built to be sat with rather than listened at: a sine crossfading into a',
        'triangle rather than a sawtooth, a filter that follows the pitch without climbing far above',
        'it, a shallow saturator, and notes that take a third of their step to arrive instead of',
        'snapping on. Reverb has a floor as well as a scale, so there is always somewhere for a sheep',
        'to sit.</p>',
        '<p><b>Tone</b> decides how sharply the variations are put. It scales the four axes that carry',
        'edge — brightness, noise, inharmonicity and fold — and opens the master filter with them,',
        'leaving the pitch mapping and the resonance alone. Low is the default and is where it is meant',
        'to live; turn it up if a sheep is too soft to read.</p>',
        '<p>Nothing on the Sound tab is saved into a sheep. Tone, volume, the drone/notes balance, note',
        'rate and scale lock are yours, like the quality settings — the sheep supplies everything',
        'else.</p>',
        '<h3>Getting it out</h3>',
        '<p><b>Record</b> captures the live view and picks the sound up with it if the sound is on.',
        '<b>Render</b> builds the file properly: the soundtrack is rendered over the same timeline as',
        'the picture, in one pass, faster than real time, so it does not matter how slow the render',
        'was. Firefox has no <code>AudioEncoder</code>, so there the soundtrack is saved beside the',
        'video as a <code>.wav</code> you can mux yourself; the Render tab says so before you',
        'start.</p>',
        '<p class="hnote">One honest limit. The loop\'s picture guarantee is exact — phase 0 and',
        'phase 1.0 differ by zero pixels. For sound the guarantee is that every control value and the',
        'note pattern close exactly, which is verified; the raw waveform is not sample-identical',
        'across the seam, because an oscillator\'s phase carries on. There is no seam to hear, but',
        'it is not the same claim.</p>'
      ].join('\n')
    },
    {
