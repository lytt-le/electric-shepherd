  ...
  var SECTIONS = [
    ...
    },
    {
      ...
      html: [
        ...
        '<tr><td><kbd>→</kbd></td><td>next sheep — while streaming it morphs across as usual; with the stream off it is a new random sheep (also a button in the top bar)</td></tr>',
        ...
        '<tr><td><kbd>H</kbd></td><td>show / hide the sheep details overlay (off by default; also a button in the top bar)</td></tr>',
        ...
        '<p>The top bar is icons rather than words, so it fits a phone as readily as a desktop. Hover any',
        'of them for its name and shortcut; the order is play/pause, next, random, mutate, fit, keep,',
        'stream, clear, the quality preset, the details overlay, fullscreen and the handbook.</p>',
        '<h3>On a phone or tablet</h3>',
        '<p>A touch device gets a shell of its own. The top bar keeps every transport button — it scrolls',
        'sideways when there are more than fit — and everything else starts hidden: no side panel, no',
        'filmstrip, no readout. The <b>☰ Options</b> button on the right slides the panel in as a drawer, up',
